...
この repository は Codex / Claude 両対応の monorepo 運用テンプレート（skills: `monorepo-bootstrap` / `harness-adopt`）です。Codex で作業する場合は、このファイルをルート入口として扱ってください。
...
| Skill | 用途 | 正本 | Claude 側入口 |
|-------|------|------|---------------|
| `monorepo-bootstrap` | 0 から repo を作る（技術選定〜deploy 検証まで） | `.agents/skills/monorepo-bootstrap/SKILL.md` | `.claude/skills/monorepo-bootstrap/SKILL.md` |
| `harness-adopt` | 既存 repo へ運用ハーネスのみ導入（既存優先・非破壊マージ） | `.agents/skills/harness-adopt/SKILL.md` | `.claude/skills/harness-adopt/SKILL.md` |

テンプレート資産の実体は `.agents/skills/monorepo-bootstrap/assets/`（台帳は `assets/MANIFEST.md`）で、両 skill が共用する。
Codex で実行するときは各正本の SKILL.md を読み、定義された手順どおり進める。
